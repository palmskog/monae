...
Module MonadTrace.
Record mixin_of T (m : choiceType -> choiceType) : Type := Mixin {
  mark : T -> m unit_choiceType
}.
Record class_of T (m : choiceType -> choiceType) : Type := Class {
  base : Monad.class_of m ; mixin : mixin_of T m }.
Structure t T := Pack { m : choiceType -> choiceType; class : class_of T m }.
Definition baseType T (M : t T) := Monad.Pack (base (class M)).
Module Exports.
Definition Mark T (M : t T) : T -> m M unit_choiceType :=
  let: Pack _ (Class _ (Mixin x)) := M return T -> m M unit_choiceType in x.
...
Definition marks T {M : traceMonad T} (l : seq T) : M [choiceType of seq unit] :=
  sequence (map Mark l).

Module MonadTraceRun.
Record mixin_of T (M : runMonad (seq T)) (mark : T -> M unit_choiceType) : Type := Mixin {
...
Structure t T : Type := Pack { m : choiceType -> choiceType ;
  class : class_of T m }.
...
Record mixin_of S T (M : monad) : Type := Mixin {
  st_get : M S ;
  st_put : S -> M unit_choiceType ;
  st_mark : T -> M unit_choiceType ;
...
Record class_of S T (m : choiceType -> choiceType) : Type := Class {
  base : Monad.class_of m ;
  mixin : mixin_of S T (Monad.Pack base)
}.
Structure t S T : Type := Pack { m : choiceType -> choiceType ; class : class_of S T m }.
...
Definition stPut S T (M : t S T) : S -> m M unit_choiceType :=
  let: Pack _ (Class _ (Mixin _ x _ _ _ _ _ _ _)) := M return S -> m M unit_choiceType in x.
Arguments stPut {S T M} : simpl never.
Definition stMark S T (M : t S T) : T -> m M unit_choiceType :=
  let: Pack _ (Class _ (Mixin _ _ x _ _ _ _ _ _)) := M return T -> m M unit_choiceType in x.
...
Variables (S : choiceType) (T : Type) (M : stateTraceMonad S T).
...
Variable M : stateTraceMonad nat_choiceType nat.

Let st_none1 : M nat_choiceType :=
  do _ <- stMark '|0|;
  do n <- stGet;
  do _ <- stPut (n + 1);
  do _ <- stMark n;
...
Variables (T : Type) (M : stateTraceMonad nat_choiceType T).
Variables (log0 log1 : T).

Definition monadtrace_example (m0 m1 m2 : M nat_choiceType) : M nat_choiceType :=
  do x <- m0;
    stPut x >>
      do y <- stGet;
        stMark log0 >>
...
Module MonadStateTraceRun.
Record mixin_of (S : choiceType) T (M : runMonad (S * seq T)) (st_get : M S)
  (st_put : S -> M unit_choiceType) (st_mark : T -> M unit_choiceType) : Type := Mixin {
...
Structure t S T : Type := Pack { m : choiceType -> choiceType ;
  class : class_of S T m }.
Definition baseType S T (M : t T S) := MonadStateTrace.Pack (base (class M)).
Module Exports.
Notation stateTraceRunMonad := t.
Coercion baseType (S : choiceType) (T : Type) (M : stateTraceRunMonad S T) : stateTraceMonad S T := baseType M.
Canonical baseType.
...
Variables (S : choiceType) (T : Type) (M : stateTraceRunMonad S T).
...
Variables (S : choiceType) (T : Type) (MN : stateTraceRunMonad S T).
...
Record class m (v : traceMonad unit) (mv : MonadTrans.t m v) : Type := Class {
  lift_ret : forall (A : choiceType) (a : A), Lift mv (Ret a) = Ret a :> v A ;
  lift_bind : forall A B (m0 : m A) (f : A -> m B),
    Lift mv (m0 >>= f) = Lift mv m0 >>= (@MonadTrans.lift _ _ mv _ \o f) :> v B ;
  drop_ret : forall (A : choiceType) (a : A), Drop mv (Ret a) = Ret a :> m A ;
...
Definition tracer_example (m0 m1 m2 : m nat_choiceType) : v _ :=
  do x <- Lift M m0;
  do y <- Lift M m1;
  Mark tt >>
  do z <- Lift M m2;
